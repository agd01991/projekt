<?php 
function getChelsList($chel, $page)
{
...
return 'Ошибка подключения к БД: '.mysqli_connect_error();
 ...
 $sql_res=mysqli_query($mysqli, 'SELECT COUNT(*) FROM std_958.'.$chel);
...
$page=$TOTAL-1; // будем выводить последнюю страницу
 $diapazon=$page*9;
$sql='SELECT * FROM '.$chel.' LIMIT '.$diapazon.', 9';
 $sql_res=mysqli_query($mysqli, $sql);
 $ret=''; // строка с будущим контентом страницы
 while( $row=mysqli_fetch_assoc($sql_res) ) // пока есть записи
{
 $ret.='<div class="card text-center"><img class="card-img-top" src="data:image/jpeg;base64,'.base64_encode($row['photo']).'" /><div class="card-body"><div class="card-title">'.$row['name'].'</div><div class="card-text"><p>'.$row['dolg'].',</p><p>'.$row['stag'].',</p><p>'.$row['spec'].'</p> <a href="#" class="btn btn-warning">Обратиться!</a></div></div></div>';
}
$ret.='</div>'; // заканчиваем формирование таблицы с контентом
 ...
 $ret.='<li class="page-item"><a class="page-link" href="?p=viewer&pg='.$i.'&chel='.$_GET['chel'].'"> '.($i+1).'</a></li>';
